...
export default function SignInPage() {
  ...
  const onOAuth = async (provider: Provider): Promise<void> => {
    ...
    try {
      ...
      setError(getErrorMessage(e) || "OAuth sign-in failed");
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md rounded-2xl border border-white/10 bg-white/10 backdrop-blur-xl shadow-2xl">
        <div className="flex justify-center pt-10">
          <div className="relative">
            <div className="h-16 w-16 rounded-2xl bg-gradient-to-r from-rose-500 to-red-500 grid place-items-center shadow-lg">
              <svg
        ...
        <div className="px-8 pt-6 text-center">
          ...
          <p className="mt-2 text-sm text-white/70">Sign in to start streaming</p>
        </div>
        ...
        <form className="px-8 pb-8 space-y-4" onSubmit={(e) => e.preventDefault()}>
          ...
          <div className="flex justify-end">
            <Link href="/forgot-password" className="text-xs text-rose-300 hover:text-rose-200 hover:underline">
              Forgot password?
            </Link>
          </div>
          ...
          <p className="pt-4 text-center text-sm text-white/70">
            ...
            <Link href="/signup" className="text-rose-300 hover:text-rose-200 hover:underline">
              Sign Up
            </Link>
          </p>
        </form>
    ...
    <div className="relative">
      ...
      <input
        ...
        className="w-full rounded-xl border border-white/15 bg-white/10 px-12 py-3
                   ...
                   focus:border-rose-400/50 focus:ring-2 focus:ring-rose-500/25"
      />
    </div>
  );
}
